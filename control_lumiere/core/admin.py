...
from .models import Zone, Permission, Schedule, ActivityLog, Lamp


@admin.register(Lamp)
class LampAdmin(admin.ModelAdmin):
    list_display = ('name', 'zone', 'user', 'status', 'gpio_pin', 'updated_at')
    list_filter = ('status', 'zone', 'user')
    search_fields = ('name', 'zone__name', 'user__username')
    list_editable = ('status', 'gpio_pin')
